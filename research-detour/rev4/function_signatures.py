...
class PluginContext:
    ...
    def on(self, event_name, fn):
        return self._bus.on(event_name, fn)

    def off(self, handle):
        return self._bus.off(handle)

    def log(self, msg):
        out(f"PLUGIN {self.name} {msg}")
...
class PluginManager:
    ...
    def load_all(self):
        ...
        for filename in filenames:
            ...
            ctx = PluginContext(self.bus, filename)
...
class Tracer:
    def __init__(self, pid, bps, watches, blocks, event_bus=None, plugin_mgr=None):
        self.pid = pid
        self.bps = bps
    ...
    def update_print_state(self, watch, value):
        if watch.name == "printing_level_mode":
            ...
            if watch.name == "lattice_busy":
                self.emit_event("printing_busy", {"busy": value})
            return
        now = time.monotonic()
    ...
    def handle_hit(self, tid, bp, pc, auto_continue=True):
        ...
        if ok_restore and ok_pc:
            ...
            temp_bp = Breakpoint(f"{bp.name}_step", temp_addr, {"mode": "thumb" if hit_is_thumb else "arm"})
    ...
    def inject_call(self, tid, spec):
        ...
        def do_inject(addr, thumb, args, vfp_saved):
            ...
            ret_bp = Breakpoint("cmd_return", return_pc & ~1, {"mode": "thumb" if return_thumb else "arm"})
...
def main():
    ...
    event_bus = EventBus()
    plugin_mgr = PluginManager(event_bus, base_dir)
    plugin_mgr.load_all()

    tracer = Tracer(pid, picked, watches, blocks, event_bus=event_bus, plugin_mgr=plugin_mgr)
